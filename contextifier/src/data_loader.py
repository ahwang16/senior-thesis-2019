...
class Data_loader:
    ...
    def __init__(self, vocab_size = 40000, max_len = 50,
        ...
        if option in ['char', 'word']:
            ...
            self.word2property = pkl.load(open(os.path.join(THIS_FOLDER, '/../model/word.pkl'), 'rb'))
            self.char2property = pkl.load(open(os.path.join(THIS_FOLDER, '/../model/char.pkl'), 'rb'))
            removed_words = []
        ...
        if option != 'both':
            self.token2property = pkl.load(open(THIS_FOLDER + '/../model/' + option + '.pkl', 'rb'))  # loading the preprocessed token file
            removed_tokens = []
        ...
        if not labeled_only:
            self.data = pkl.load(open(THIS_FOLDER + '/../data/data.pkl', 'rb'))
        else:
            self.data = pkl.load(open(THIS_FOLDER + '/../data/labeled_data.pkl', 'rb'))
