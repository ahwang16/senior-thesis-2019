...
def kmeans(embeds, vocab, k=900, r=25, file_num=0) :
	...
	print("pickling")
	with open("../data/kmeans_clusters_w2v_{}.pkl".format(file_num), "wb") as p :
		pkl.dump(cluster_dict, p)

	############################################################################

	# write individual precision and recall scores to text file
	f = open("../data/kmeans_w2v_{}.txt".format(file_num), "w")
	...
	scores = open("../data/kmeans_scores_w2v_{}.txt".format(file_num), "w")
...
def agglom(embeds, vocab, affinity="cosine", linkage="average", num_clusters=900, file_num=0) :
	...
	print("pickling")
	with open("../data/agglom_clusters_w2v_{}.pkl".format(file_num), "wb") as p :
		pkl.dump(cluster_dict, p)

	############################################################################

	# write individual precision and recall scores to text file
	f = open("../data/agglom_w2v_{}.txt".format(file_num), "w")
	f.write("vocab\tprecision\trecall\n")
	...
	scores = open("../data/agglom_scores_w2v{}.txt".format(file_num), "w")
	scores.write("cluster\tprecision\trecall\n")
